...
GODOT="${GODOT_BIN:-${HOME}/Downloads/Godot_v4.6.1-stable_linux.x86_64}"
ORT_ROOT="${ORT_ROOT:-/tmp/onnxruntime-linux-x64-1.20.1}"
OUT="${OUT:-/tmp/godot-onnx-loader-csv-smoke.txt}"

if [[ ! -x "$GODOT" ]]; then
	echo "GODOT_BIN must point at Godot 4.x (default: ~/Downloads/Godot_v4.6.1-stable_linux.x86_64)" >&2
	exit 1
fi
...
if grep -q 'free(): invalid size' "$OUT"; then
	echo "FAIL: glibc free error in $OUT" >&2
	exit 1
fi
